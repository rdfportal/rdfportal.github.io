(function () {
  ...
  var HomeViewController = /*#__PURE__*/function () {
    ...
    _createClass(HomeViewController, [{
      key: "show",
      value: function show() {
        var i, j, y, radian1, radian2, radian3, radius; // corn

        if (!this._isInitialized) {
          this._isInitialized = true;
          this._d3Corn = __g.d3.svg.append('g').attr('class', 'corn'); // line
  ...
  var ListViewController = /*#__PURE__*/function () {
    ...
    _createClass(ListViewController, [{
      ...
      value: function makeList() {
        ...
        __g.app.addNotificationObserver('change-filtering-tags', this, this.receiveNotificationChangeFilter); // ソート


        sortedIds = _sort(__g.controlPanel.kindOfSorting(), __g.controlPanel.isAscend()); // サイズ種別を取得し各ノードに通知、レイアウトを行う

        this.placeStars(sortedIds); // ウインドウリサイズで星の矩形のリサイズ
  ...
  var MatrixItemView = function MatrixItemView(colItem, order, delegate) {
  ...
  var MatrixViewController = /*#__PURE__*/function () {
    function MatrixViewController() {
      ...
      this._sortedIds; //this._itemKeys;
    ...
    _createClass(MatrixViewController, [{
      ...
      value: function show() {
        ...
        if (!this._isInitialized) {
          this._isInitialized = true;
          this._sortedColItems = []; //this._itemKeys = {};
          // ソート

          this._sortBy = __g.userDefaults.matrix.sortBy;
          this._sortedIds = _sort(this._sortBy, true); // キーのソート

          this._sortedColItems = __g.userDefaults.matrix.colItems; // 表組みの作成

          this.makeMatrix(); // scroll bar
      ...
      value: function makeMatrix() {
        ...
        this._$body = $('#matrix-body'); // セルオブジェクト生成
        ...
        this._$colItems = this._$matrix.find('div.cell.item-Alphabetical, div.inner header div.cell'); // 合計値

        var height = ORIGIN$1.Y + ids.length * CELL.HEIGHT;

        this._$body.find('.id-sum').css('top', height); // padding
        ...
        this._matrixOverallWidth += PADDING_RIGHT; // comma

        _placeComma(this._$body); // events
        // sort
        ...
        this._$colItems.on('click', function (e) {
          ...
          if (e.delegateTarget.classList.contains('selected')) {
            ...
            // 現在選択中のセルを解除
            _this2._$colItems.filter("[data-sort=\"".concat(_this2._sortBy, "\"]")).removeClass('selected'); // セルの選択
      ...
      value: function placedStar() {
        if (--this._placedStarCoutner === 0) {
          ...
          __g.app.changed(__g.VIEWS_NAME.indexOf('statistics')); // scroll bar
      ...
      value: function hide() {
        ...
        __g.app.$star.off('mouseenter.matrix mouseleave.matrix'); //this._scrollBar = null;
  ...
  var GraphViewController = /*#__PURE__*/function () {
    ...
    _createClass(GraphViewController, [{
      key: "show",
      value: function show() {
        this.receiveNotificationChangeSizing(); // tick
        ...
        $w.trigger('resize.mainViewControl'); // イベントのバインド
      ...
      value: function receiveNotificationChangeSizing() {
        ...
        __g.d3.force.force('collide', // ノード間の接触反発力
      ...
      value: function show(params) {
        ...
        $.ajax({
          ...
          var _loop = function _loop(lang) {
            ...
            $container.html('');
            var section = document.createElement('section'); // HTMLElement を回してレンダリング
  ...
  var ChangeLogViewController = /*#__PURE__*/function () {
    ...
    _createClass(ChangeLogViewController, [{
      ...
      value: function show(params) {
        ...
        if (!this._isInitialized) {
          this._isInitialized = true; // rendering
  ...
  var StarImage = /*#__PURE__*/function () {
    ...
    _createClass(StarImage, [{
      ...
      value: function generateIconFromText() {
        ...
        acronym = acronym.toUpperCase(); // canvasでイメージの生成
      ...
      value: function generateIconFromImg() {
        var canvas, ctx, dx, dy, dw, dh; // canvasでイメージの生成
  ...
  var DetailViewController = /*#__PURE__*/function () {
    ...
    _createClass(DetailViewController, [{
      ...
      value: function makeDetailView() {
        ...
        }, datasets.downloadFile.name(starId)) + ' ' + _html('small', _numeral(datasets.downloadFile.size(starId)) + ' bytes')))
        /* + // ガイドライン一時的にコメントアウト
          ...
          if (noOfConnectedDatasets > 0) {
            ...
            subHtml1 += _html('aside', {
              ...
              var subHtml2 = '',
                  ...
                  noOfXrefs = datasets.xref.noOf(starId); // calculation
        ...
        _MLHtml('h3', STR.statistics) + _html('table', {
          class: 'horizontal-table'
        }, _htmlStatics(starId) + _htmlHasDetailAndDetail('graphs') + // Graphs
        ...
        this._$currentDetailView = $('#detail-view' + this._counter++); //$detailView = __g.append

        __g.app.$main.children('.detail-view');

        _$schemaImages = $('#detail-schema').children('.images'); // comma

        _placeComma(this._$currentDetailView); // CodeMirror
        ...
        }).triggerHandler('scroll.detail-navigation'); // 関連データセットのノード
        ...
        _setDetailCollapsible(this._$currentDetailView); // 折りたたまれたテーブルの生成
        ...
        var _makeCollapsedTable = function _makeCollapsedTable(DATASETS, classification, jsonKind, index, offset, $tbody) {
          var subHtml = '',
              ...
              noOfDomainClasses,
              noOfRangeClasses,
              noOf = DATASETS.noOf(starId, index); // remove indicator

          $tbody.find('tr.loading, tr.more').remove(); // html
  ...
  */
  ...
  var ViewLabel = /*#__PURE__*/function () {
    function ViewLabel() {
      ...
      this._index;
      this._changingLable;
      this._$label = $('#view-label'); // メニューにマウスが載るとラベルを変更
    ...
    _createClass(ViewLabel, [{
      ...
      value: function changingLabel() {
        ...
        if (remainder !== 0) {
          changingLable = "".concat(changingLable.slice(0, -1), "<span style=\"color: hsl(").concat(Math.random() * 360, ", 100%, ").concat(100 * (remainder / _DURATION), "%);\">").concat(String.fromCharCode(parseInt(97 + 25 * Math.random())), "</span>"); //changingLable = `${changingLable.slice(0, -1)}<span style="color: hsl(240, 50%, ${100 * ((remainder) / _DURATION)}%);">${String.fromCharCode(parseInt(97 + 25 * Math.random()))}</span>`;
        }
  ...
  var ControlPanel = /*#__PURE__*/function () {
    function ControlPanel() {
      ...
      (function () {
        ...
        _$tagMarker.append(htmlMarker); // マーカータグを格納
  ...
  var Star = /*#__PURE__*/function () {
    function Star(_elm, _data) {
      ...
      this._mouseDownPosition; // initialize
      ...
      if (this._isInternal) {
        ...
        this._$.addClass('star' + function () {
          ...
          for (var _i2 = 0; _i2 < _this._datasets.tags.noOf(_this._id); _i2++) {
            tagClass += " tag-".concat(_sanitizeTag(_this._datasets.tags.tag(_this._id, _i2).en));
          }
        ...
        this._$numberOf = $('.number-of', this._$); // リンク情報の付与

        var _i;

        for (_i = 0; _i < this._datasets.connectedDatasetIds.noOf(this._id); _i++) {
          this._$.addClass('to-' + this._datasets.connectedDatasetIds.id(this._id, _i));
        }

        for (_i = 0; _i < this._datasets.xref.noOf(this._id); _i++) {
          this._$.addClass('to-' + __g.app.xrefs[this._datasets.xref.title(this._id, _i)].key);
        } // logotype
        ...
        for (_i = 0; _i < this._datasets.tags.noOf(this._id); _i++) {
          this._tags.push(_sanitizeTag(this._datasets.tags.tag(this._id, _i).en));
        }
      } else {
        // 外部データベースの場合
    ...
    _createClass(Star, [{
      ...
      value: function prepareListView(order, top, delegate) {
        ...
        if (this._isInternal) {
          ...
          this._$.removeClass('hidden'); // size
          ...
          this._$.animate({
            ...
            complete: function complete() {
              _this2._$.addClass('placed'); // make information
      ...
      value: function prepareGraphView() {
        ...
        if (this._isInternal) {
          ...
          this.receiveNotificationChangeTagMarker(__g.controlPanel.selectedMarkedTag()); // コンパネのタグにマウスが当たると、同じタグを持つ星がハイライト
    ...
    _createClass(RDFPortal, [{
      ...
      value: function render(kindOfView, params) {
        ...
        if (kindOfView === __g.VIEWS.dataset || kindOfView === __g.VIEWS.documents || kindOfView !== this._currentView) {
          ...
          this._currentView = kindOfView; // スクリーンに属性付与

          $('body').attr('data-kind-of-view', __g.VIEWS_NAME[kindOfView]);
          this.$main.attr('data-kind-of-view', __g.VIEWS_NAME[kindOfView]); // 星たちに通知
          ...
          this.viewLabel.changeLable(__g.VIEWS_LABEL[this._currentView]); //_$navLabel.text( __g.VIEWS_LABEL[this._currentView] );
          ...
          this.currentViewController.show(params); // 遷移状態に
      ...
      value: function init(data) {
        ...
        this._appPrepareCounter = this.datasets.noOfDatasets() + 1; // Stylesheet

        newStyle = document.createElement('style');
        newStyle.type = 'text/css';
        document.getElementsByTagName('head').item(0).appendChild(newStyle);
        __g.styleSheet = document.styleSheets[document.styleSheets.length - 1]; // SVG

        __g.d3.edges = __g.d3.svg.append('g').attr('class', 'edges'); // 各ビューのコントローラ生成
        ...
        this.currentViewController = this._homeViewController; // ナビゲーション：ビュー切り替え
        ...
        $('#language-switch').find('li[data-lang="' + this.language + '"]').trigger('click'); // コンソールパネル

        __g.controlPanel = new ControlPanel(); // データセットの基本情報の取得
        ...
        }).trigger('resize.mainViewControl'); // force layout

        {
          ...
          this.$star = $('main#main-view').children('.stars').children('.star'); // リンク（エッジ）の生成
          ...
          __g.d3.linksSelection.each(function () {
            ...
            d3This.classed('edge ' + classed, true); // triple number
          ...
          d3.forceLink() //.distance(distance) // リンクの長さ
          //.strength(.5) // リンクの強度
          //.iterations() // 計算回数
          ...
          d3.forceManyBody() //.strength(-300) // 正でくっつき、負で離れる
          //.theta() // 精度
          ...
          __g.d3.force.nodes(__g.d3.nodesData); // エッジ


          __g.d3.force.force('link').links(__g.d3.linksData); // force layout のパラメータ確認用のコンソール


          $('#force-layout-console').find('input').each(function () {//this.value = __g.d3.force[this.dataset.key]();
          }).on('change', function () {//__g.d3.force[this.dataset.key]( parseFloat(this.value) ).resume();
          }); // ドラッグ可能に
  ...
  /*global $, d3 */
  $(function () {
    __g.d3.svg = d3.select('main#main-view').append('svg').attr('id', 'svg'); // ガイドラインの取得
    ...
    function init(data) {
      ...
      __g.app.init(data); // ウインドウを閉じる際に諸情報を保存
